    ...
    <div className="flex flex-col gap-4 pb-10">
      ...
      <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-3xl p-4 sm:p-5
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="bg-white/20 rounded-2xl px-3 py-2 sm:px-4 sm:py-3 text-center shrink-0">
      ...
      {/* ── Subject grid — 2 kolom di semua ukuran layar ── */}
        ...
        {SUBJECTS.map(s => {
          ...
          return (
            <button key={s.id}
              ...
              className={`group bg-white border-2 ${m.border} rounded-2xl sm:rounded-3xl
                         p-4 text-left shadow-sm hover:shadow-md hover:-translate-y-1
                         active:translate-y-0 active:scale-95 transition-all duration-200`}>

              {/* Icon */}
              <div className={`${m.bg} w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl
                               flex items-center justify-center text-xl sm:text-2xl
                               shadow-sm group-hover:scale-110 transition-transform mb-3`}>
                {m.icon}
              </div>

              {/* Title */}
              <h3 className="font-title text-sm sm:text-base text-slate-700 leading-tight">
                {s.name}
              </h3>
              <p className="text-slate-400 text-[10px] sm:text-xs font-bold mt-0.5 leading-relaxed">
                {m.desc}
              </p>

              {/* Progress */}
              <div className="mt-2 sm:mt-3">
                <div className={`w-full ${m.light} rounded-full h-2 overflow-hidden border ${m.border}`}>
                ...
                <div className="flex justify-between mt-1">
                  ...
                  <span className={`text-[9px] sm:text-[10px] font-bold ${m.text}`}>
                    {pct}%
                  </span>
                </div>
              </div>
      ...
      <button
        ...
        className="bg-gradient-to-r from-purple-500 to-indigo-500 rounded-2xl p-4
      ...
      >
        ...
        <div>
          <p className="font-title text-base text-white">Ruang Game</p>
