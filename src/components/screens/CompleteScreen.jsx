...
export default function CompleteScreen() {
  ...
  const accuracy = wrongCount === 0 ? 100 : Math.round((1 - wrongCount/6) * 100);

  return (
    <div className="flex flex-col items-center gap-5 py-8 max-w-md mx-auto text-center px-4 animate-slide-up">
      ...
      {/* ── Title ── */}
      <div>
        <h1 className="font-title text-4xl text-slate-800">
      ...
      <div className="grid grid-cols-2 gap-3 w-full">
        <StatCard icon="⚡" label="XP Didapat"  value={`+${xpEarned}`}   color="blue"/>
        <StatCard icon="🎯" label="Akurasi"      value={`${accuracy}%`}   color="green"/>
        <StatCard icon="⭐" label="Bintang"      value={`${saved} / 3`}   color="amber"/>
        <StatCard icon="🔥" label="Streak"       value={`${streak} hari`} color="orange"/>
      </div>

      {/* ── Perfect bonus ── */}
      {perfect && (
        <div className="w-full bg-gradient-to-r from-yellow-50 to-amber-50
                        border-2 border-amber-200 rounded-2xl p-4 animate-pop-in">
          <p className="font-title text-base text-amber-600">🏆 Bonus Sempurna!</p>
          <p className="text-amber-500 text-xs font-bold mt-0.5">Kamu menjawab semua soal dengan benar!</p>
        </div>
      )}
      ...
      <div className="flex gap-3 w-full">
        <button onClick={() => goTo('subject')}
                className="flex-1 py-4 rounded-2xl font-title text-base
                           ...
                           hover:border-slate-300 hover:-translate-y-0.5 transition-all">
          🏠 Beranda
        </button>
...
function StatCard({ icon, label, value, color }) {
  ...
  return (
    <div className={`${colors[color]} border-2 rounded-2xl p-4 text-center`}>
      <p className="text-2xl mb-1">{icon}</p>
      <p className={`font-title text-xl ${colors[color].split(' ')[2]}`}>{value}</p>
