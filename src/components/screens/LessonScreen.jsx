...
const WRONG_MSG   = ['Hampir benar! 💪','Jangan menyerah! 🌟','Robi percaya kamu bisa!','Belajar dari salah! 🧠'];
...
function MiniRobi({ emotion = 'happy' }) {
  ...
  return (
    <svg viewBox="0 0 100 120" width="48" height="58" style={{display:'block',flexShrink:0}}>
...
const TIMER_SECONDS = { choice:15, match:30, word_builder:25, type_answer:20, picture_quiz:12 };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-50 flex flex-col">
      ...
      <div className="bg-white border-b border-slate-100 px-3 sm:px-4 py-3
                      flex items-center gap-2 sm:gap-3 shadow-sm sticky top-0 z-10">
        <button onClick={()=>{ sfxClick(); goHomeFromLesson(); }}
                className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center
                           text-slate-400 hover:bg-red-50 hover:text-red-400
                           active:scale-95 transition-all font-bold shrink-0 text-lg">
          ✕
        </button>
        ...
        <div className="flex-1 flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-slate-400 shrink-0 w-9 text-right">
            {qIndex}/{total}
          </span>
        </div>
        ...
        <div className="flex gap-0.5 shrink-0">
          {Array.from({length:maxHearts}).map((_,i)=>(
            <span key={i} className={`text-base sm:text-xl transition-all duration-300
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain">
        <div className="max-w-xl mx-auto px-3 sm:px-4 py-4 sm:py-6 flex flex-col gap-4">

          {/* Type badge */}
          <span className={`self-start flex items-center gap-1.5 text-xs font-bold
                            px-3 py-1.5 rounded-full border-2 ${typeBadge.color}`}>
            {typeBadge.icon} {typeBadge.label}
          </span>

          {/* Timer */}
          {qtype==='choice' && !checked && (
            <QuestionTimer key={timerKey} seconds={timerSec}
                           onTimeout={handleTimeout} paused={checked}/>
          )}

          {/* Question text */}
          {!isMatch && !isWord && !isPic && (
            <h2 className="font-title text-xl sm:text-2xl text-slate-800 leading-snug">
              {q.question}
            </h2>
          )}

          {/* Robi speech bubble */}
          <div className={`flex items-end gap-3 p-3 sm:p-4 rounded-2xl border-2
            ...
            <div className="flex-1 min-w-0">
              {!checked ? (
                ...
                <div>
                  <p className={`font-title text-sm sm:text-base
                                 ${correct?'text-green-600':'text-red-500'}`}>
                    {correct?'✅ Betul!':'❌ Salah!'}
                  </p>
                  <p className="text-xs font-bold text-slate-500 mt-0.5">{fbMsg}</p>
                  {!correct && q.answer && (
          ...
          {/* Question visual */}
          {!isMatch && !isWord && !isPic && <QuestionDisplay question={q}/>}

          {/* Match */}
          {isMatch && !matchDone && <MatchQuestion pairs={q.pairs} onAllMatched={handleMatchDone}/>}
          {isMatch && matchDone  && (
            <div className="text-center py-6 font-title text-xl sm:text-2xl text-green-600 animate-pop-in">
              ...
              {q.options.map(opt=>{
                ...
                if(!checked){
                  cls = opt.value===selected
                    ...
                    : 'bg-white border-slate-200 border-b-slate-300 text-slate-700 hover:border-blue-300 hover:bg-blue-50 hover:-translate-y-0.5 active:scale-95';
                } else {
                ...
                return (
                  <button key={opt.value} disabled={checked}
                          ...
                          className={`p-3 sm:p-4 rounded-2xl border-2 border-b-4 font-bold
      ...
      {/* ── Footer — choice only ── */}
      {qtype==='choice' && (
        <div className={`border-t-2 px-3 sm:px-4 py-3 sm:py-4 transition-colors duration-300
          ...
          <div className="max-w-xl mx-auto flex justify-end">
            {!checked ? (
              <button onClick={check} disabled={selected===null}
                      className={`px-8 sm:px-10 py-3.5 sm:py-4 rounded-2xl font-title
                                  text-base sm:text-lg text-white border-b-4 transition-all
              ...
              <button onClick={nextQ}
                      className={`px-8 sm:px-10 py-3.5 sm:py-4 rounded-2xl font-title
                                  text-base sm:text-lg text-white border-b-4 transition-all
                                  min-h-[52px] min-w-[120px]
                                  hover:-translate-y-0.5 active:translate-y-0 active:border-b-2 shadow-lg
